...
def main():
    ...
    for vehicle in range(manager.GetNumberOfVehicles()):
        ...
        makespan_by_vehicle.append(distance_dimension.CumulVar(end_index))

    for l in range(len(data["distance_matrix"])):
        l_index = manager.NodeToIndex(l)
        routing.solver().Add(
            capacity_dimension.CumulVar(l_index)
            <= Foo_Constant
            - routing.solver().Element(makespan_by_vehicle, routing.VehicleVar(l_index))
        )
    ##################################
